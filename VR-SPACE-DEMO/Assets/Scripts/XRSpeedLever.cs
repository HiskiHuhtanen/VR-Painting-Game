...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class XRLever : XRBaseInteractable
    {
        ...
        void Start()
        {
            // Start with lever at 0 degrees instead of snapped to min/max
            SetHandleAngle(0f);
        }
        ...
        void OnValidate()
        {
            // Only set handle angle in editor if we're not playing
            if (!Application.isPlaying)
            {
                SetHandleAngle(0f); // Start at neutral position
            }
        }
    }
}
